...
/*
 * 1. в базе 4 вагон у всех тележек изменить с гидравлики на убрать за габарит пути

2. поправить боксы линейки сделать их соразмерными тем местам куда мы эту линейку прикладываем и чтобы она попадала под колесо и мерила расстояние от колеса до рельсы.

3.  свободные автосцепки не отмечаем никак! просто отключить их логику

4. выбираем длинну пути стрелками подсвечиваем и затем подсвечиваем саму рельсу показываем что она разрушена на расстоянии 150 метров к примеру

5. починить баг со странным выделением пунктов которые висят на руке они багуются

6. когда выбрали вагон он 10 секунд не активен для выделения чтоб случайно дважды его не кликнуть

7. что то решить со самопроизвольными выделениями когда телепортируемся и цепляем при этом тележку

8. доперерисовать график работ посередине сделать все красиво и крупно

9. сверху от этого графика работ сделать финальный таймер сколько осталось времени до конца

10.  проработать концовку, чтобы когда мы закончили прохождение и появился самый финальный план мы слышали что мы победили

11. путь выбираем в двух местах и показывается плашка сколько метров повреждено пути

12. при выборе автосецпки появляется статичная плашка и на ней написано зажата эта автосцепка или нет

13. ограничители так чтобы не льзя было телепортироватся в центр вагона рассставить под ними мертвые плоские коллайдеры

14. при составлении плана АВР уменьшить вагончики чтобы они все влезали. убрать стрелки

15. доработать логику вагонов фронтов

16.  1. Кран ВП (осн) 2. АВЛ (вспом) 3. Бульдозер ВП (осн) 4. АВЛ (осн)

17. записать все звуки для всех плашек

18. Поздравляем Составленый план является оптимальным

19. К сожалению составленный план АВР не является оптимальным

время открытия движения по соседнему пути превысит 19:00 (на накрасном фоне)  мини кнопочка составить план АВР заново)) кнопка
 * */
...
using GrabberPool = HTC.UnityPlugin.Utility.ObjectPool<Selectable.Grabber>;

// demonstrate of dragging things useing built in EventSystem handlers
public class Selectable : GrabbableBase<PointerEventData, Selectable.Grabber>
...
{
    [Serializable]
    public class UnityEventDraggable : UnityEvent<Selectable> { }
